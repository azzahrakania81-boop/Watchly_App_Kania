...
class SupabaseService {
  ...
  Future<List<Watchlist>> getWatchlists() async {
    final response = await client.from('watchlists').select();
  ...
  Future<void> addWatchlist(Watchlist watchlist) async {
    await client.from('watchlists').insert(
      watchlist.toJson(),
    );
  }

  Future<void> updateWatchlist(Watchlist watchlist) async {
    await client.from('watchlists').update(
      watchlist.toJson(),
    ).eq(
      'id',
      watchlist.id!,
  ...
  Future<void> deleteWatchlist(String id) async {
    await client.from('watchlists').delete().eq(
      'id',
      id,
    );
  }
